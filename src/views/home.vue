    ...
    <div v-else class="koi-body">
      ...
      <div class="header">
        <ul class="link-list left">
          ...
          <li>
            <router-link to="/trace"> 溯源统计 </router-link>
          </li>
          <li>
            <router-link to="/monitor"> 智能监控 </router-link>
          </li>
          <li>
            <router-link to="/data-statistics"> 数据统计 </router-link>
          </li>
        </ul>
        <ul class="link-list right">
          ...
          <li>
            <router-link to="/"> 环境监测 </router-link>
          </li>
          <li>
            <router-link to="/"> 生产管理 </router-link>
          </li>
        </ul>
        <!-- 时间 -->
        ...
        <h1
          ...
          :style="{ 'font-size': Math.round(this.screenWidth / 60) + 'px' }"
        >
          食用菌生产全程可视化管理系统
        </h1>
        <!-- 装饰8 -->
    ...
    screenWidth: function (news, old) {
      if (news <= 1200) {
        this.$notify({
          ...
          message:
            '<strong style= "color: teal">浏览器宽度<1200px将不再进行自适应</strong>',
          type: "warning",
    ...
    screenHeight: function (news, old) {
      if (news <= 700) {
        this.$notify({
          ...
          dangerouslyUseHTMLString: true,
          message:
            '<strong style= "color: teal">浏览器高度<700px将不再进行自适应</strong>',
          type: "warning",
          position: "bottom-right",
          duration: 2000,
        });
